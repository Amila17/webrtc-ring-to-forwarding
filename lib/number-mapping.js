...
module.exports = function(logger) {
  function initMap() {
    ...
    logger.info(`configured ${idx} VoxDIDs for ${custMap.size} customers`);
  }
  ...
  return function getRoutingForDid(did) {
    if (numberMap.has(did)) {
      ...
      if (custMap.has(info.customer)) {
        ...
        logger.info(`unknown customer ${info.customer}`);
      }
    }
    else {
      logger.info(`unknown DID ${did}`);
    }
  };
